    ...
    document.getElementById("item_id").addEventListener("change", function () {
        ...
        if (itemId !== "") {
            fetch("fetch_units_by_item.php?item_id=" + encodeURIComponent(itemId))
                ...
                .then(data => {
                    if (data.length > 0) {
                        let html = '<table id="gridTable" class="table table-bordered table-striped">';
                        html += '<thead><tr><th>Packing Unit Name</th><th>Rent Per Month</th><th>Rent Per Seasonal</th></tr></thead>';
                        html += '<tbody>';
                        data.forEach(unit => {
                            html += `<tr>
                                <td>${unit.packing_unit_name}</td>
                                <td>${unit.rent_per_month}</td>
                                <td>${unit.rent_per_seasonal}</td>
                            </tr>`;
                        });
                        html += '</tbody></table>';
