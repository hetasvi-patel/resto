...
if (isset($_REQUEST['action'])) {
    ...
    if ($action === 'fetch_units' && isset($_REQUEST['item_id']) && isset($_REQUEST['customer_id'])) {
        ...
        try {
            $stmt = $_dbh->prepare("
                SELECT 
                    ...
                    COALESCE(cipplm.rent_per_qty_month, ipplm.rent_per_qty_month, '0.00') AS rent_per_qty_month, 
                    COALESCE(cipplm.rent_per_qty_season, ipplm.rent_per_qty_season, '0.00') AS rent_per_qty_season,
                    CASE 
                        WHEN cipplm.rent_per_qty_month IS NOT NULL OR cipplm.rent_per_qty_season IS NOT NULL THEN 1 
                        ELSE 0 
                    END AS is_updated
                FROM tbl_packing_unit_master pum
                LEFT JOIN tbl_item_preservation_price_list_master ipplm
                    ON pum.packing_unit_id = ipplm.packing_unit_id
                    AND ipplm.item_id = :item_id
                LEFT JOIN tbl_customer_wise_item_preservation_price_list_master cipplm
                    ON pum.packing_unit_id = cipplm.packing_unit_id
                    AND cipplm.item_id = :item_id
                    AND cipplm.customer_id = :customer_id
                WHERE pum.status = 1
            ");
            ...
            error_log("Database Error (fetch_units): " . $e->getMessage());
    ...
    foreach ($data as $unit) {
        try {
            ...
            $stmt = $_dbh->prepare("
                ...
                SET 
                    rent_per_qty_month = :rent_per_qty_month,
                    rent_per_qty_season = :rent_per_qty_season,
            ...
            ");
            $stmt->bindParam(':rent_per_qty_month', $unit['rent_per_qty_month'], PDO::PARAM_STR);
            $stmt->bindParam(':rent_per_qty_season', $unit['rent_per_qty_season'], PDO::PARAM_STR);
            $stmt->bindParam(':packing_unit_id', $unit['packing_unit_id'], PDO::PARAM_INT);
            $stmt->bindParam(':item_id', $unit['item_id'], PDO::PARAM_INT);
            $stmt->bindParam(':customer_id', $unit['customer_id'], PDO::PARAM_INT);
            $stmt->bindParam(':company_id', $unit['company_id'], PDO::PARAM_INT);
            $stmt->execute();
            if ($stmt->rowCount() === 0) {
                $insertStmt = $_dbh->prepare("
                    INSERT INTO tbl_customer_wise_item_preservation_price_list_master 
                        (packing_unit_id, item_id, customer_id, rent_per_qty_month, rent_per_qty_season,company_id)
                    VALUES 
                        (:packing_unit_id, :item_id, :customer_id, :rent_per_qty_month, :rent_per_qty_season,:company_id)
                ");
                $insertStmt->bindParam(':packing_unit_id', $unit['packing_unit_id'], PDO::PARAM_INT);
                $insertStmt->bindParam(':item_id', $unit['item_id'], PDO::PARAM_INT);
                $insertStmt->bindParam(':customer_id', $unit['customer_id'], PDO::PARAM_INT);
                $insertStmt->bindParam(':rent_per_qty_month', $unit['rent_per_qty_month'], PDO::PARAM_STR);
                $insertStmt->bindParam(':rent_per_qty_season', $unit['rent_per_qty_season'], PDO::PARAM_STR);
                $insertStmt->bindParam(':company_id', $company_id, PDO::PARAM_INT);
                $insertStmt->execute();
            }
        } catch (PDOException $e) {
            error_log("Database Error (save_unit): " . $e->getMessage());
            $success = false;
        }
    }
